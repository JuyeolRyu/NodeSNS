  ...
  return produce(state,(draft) => {
    switch (action.type) {
      ...
      case LOAD_FOLLOWERS_REQUEST: {
        draft.followerList = !action.offset ? [] : draft.followerList; //offset이 0(처음 로드)할때 빈 배열
        draft.hasMoreFollower = action.offset ? draft.hasMoreFollower : true;//처음 데이터를 가져 올때는 더보기 버튼 true
        break;
      }
      case LOAD_FOLLOWERS_SUCCESS: {
        //concat은 forEach로 push
